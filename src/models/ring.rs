...
impl Ring {
    ...
    pub fn render(&self, c: Context, g: &mut G2d) {
        CircleArc::new(GameColors::Red.into(),
                       self.thickness,
                       PI * 0.0 + self.direction,
                       PI * 0.5 + self.direction)
            .resolution(256)
            .draw([-self.radius, -self.radius, self.radius * 2.0, self.radius * 2.0],
                  &c.draw_state,
                  c.transform,
                  g);

        CircleArc::new(GameColors::Green.into(),
                       self.thickness,
                       PI * 0.5 + self.direction,
        ...
        CircleArc::new(GameColors::Blue.into(),
                       self.thickness,
                       PI * 1.0 + self.direction,
        ...
        CircleArc::new(GameColors::Yellow.into(),
                       self.thickness,
                       PI * 1.5 + self.direction,
    ...
    pub fn get_color_at_radian(&self, theta: f64) -> GameColors {
        ...
        if Ring::is_in_bounds(unit_theta, self.red_bounds()) {
            return GameColors::Red;
        }

        if Ring::is_in_bounds(unit_theta, self.green_bounds()) {
            return GameColors::Green;
        }

        if Ring::is_in_bounds(unit_theta, self.blue_bounds()) {
            return GameColors::Blue;
        }

        if Ring::is_in_bounds(unit_theta, self.yellow_bounds()) {
